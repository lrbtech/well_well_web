            ...
            <div class="page-header">
              <div class="row">
                <div class="col-lg-6 main-header">
                  <h2>{{$language[3][Auth::guard('admin')->user()->lang]}} <span>{{$language[129][Auth::guard('admin')->user()->lang]}}  </span></h2>
              ...
              <div class="col-sm-12">
                <div class="card">
                  <div class="card-header">
                    ...
                    @if(Auth::guard('admin')->user()->station_create == 'on')
                    <button id="add_new" style="width: 200px;" type="button" class="btn btn-primary add-task-btn btn-block my-1">
                    <i class="bx bx-plus"></i>
                    <span>{{$language[130][Auth::guard('admin')->user()->lang]}}</span>
                    </button>
                    @endif
                  </div>
                  <div class="card-body">
                    <div class="table-responsive">
                      <table class="display" id="basic-1">
                        <thead>
                          <tr>
                            <th>#</th>
                            <th>{{$language[131][Auth::guard('admin')->user()->lang]}}</th>
    ...
    <div class="modal-content">
        ...
        <div class="row">
            <div class="form-group col-md-12">
              <label class="col-form-label">{{$language[222][Auth::guard('admin')->user()->lang]}}</label>
              <input autocomplete="off" type="text" id="station" name="station" class="form-control">
            </div>
        </div>
...
function Delete(id,status){
    ...
    if (r == true) {
      $.ajax({
        url : '/admin/station-delete/'+id+'/'+status,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
