              ...
              <div class="col-sm-12">
                <div class="card">
                  <div class="card-header">
                    ...
                    @if(Auth::guard('admin')->user()->package_category_create == 'on')
                  ...
                  <div class="card-body">
                    <div class="table-responsive">
                      <table class="display" id="basic-1">
                        ...
                        <tbody>
                        @foreach($package_category as $key => $row)
                          <tr>
                            <td>{{$key+1}}</td>
                            <td>{{$row->category}}</td>
                            <td>
                            ...
                            <td>
                                ...
                                @if(Auth::guard('admin')->user()->package_category_edit == 'on')
                                    <a onclick="Edit({{$row->id}})" class="dropdown-item" href="#">{{$language[225][Auth::guard('admin')->user()->lang]}}</a>
                                    @endif
                                    @if(Auth::guard('admin')->user()->package_category_delete == '0')
                                    @if($row->status == 0)
    ...
    <div class="modal-content">
        ...
        <div class="row">
            <div class="form-group col-md-12">
              <label class="col-form-label">{{$language[229][Auth::guard('admin')->user()->lang]}}</label>
...
function Save(){
  ...
  if(action_type == 1){
    $.ajax({
        url : '/admin/save-package-category',
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
    ...
    $.ajax({
      url : '/admin/update-package-category',
      type: "POST",
      data: formData,
      contentType: false,
      processData: false,
...
function Edit(id){
  $.ajax({
    url : '/admin/edit-package-category/'+id,
    type: "GET",
    dataType: "JSON",
    success: function(data)
    {
